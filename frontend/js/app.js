...
async function refreshAllData() {
  try {
    ...
    // Only ensure citizen-reported complaints have an associated work order
    // Never flood the work orders with hundreds of benchmark demo hazards
    for (const h of allHazards) {
      const isCitizen = Boolean((h.id && /^HAZ-\d{6}-/.test(h.id)) || h.citizen_report);
...
async function deleteWorkOrder(orderId) {
  ...
  try {
    ...
    // Fallback: remember cancelled locally so UI stays responsive
    rememberCancelledWorkOrder(orderId, targetOrder?.target_hazard_ids || []);
    allWorkOrders = allWorkOrders.filter(wo => wo.id.toUpperCase() !== orderId.toUpperCase());
    applyStateAndSearchFilters();
    showToastNotification(`Work Order ${orderId} cancelled locally.`);
  }
}
...
function switchTab(tabId) {
  ...
  if (tabId === 'studio') {
    setTimeout(() => {
      if (typeof loadStudioScenario === 'function') loadStudioScenario('pothole');
  ...
  if (tabId === 'copilot') {
    if (typeof syncCopilotDirectApiKeyStatus === 'function') syncCopilotDirectApiKeyStatus();
  }
